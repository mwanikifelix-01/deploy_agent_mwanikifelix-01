...
To access it, first clone the repository using the link: To run it, you make the script executable with `chmod +x setup.sh`, run it with `./setup.sh`, enter your preferred thresholds when prompted, then navigate into the generated folder and run `python3 attendance_checker.py`. The checker reads the student CSV, calculates each student's attendance percentage against the total sessions in the config, and writes alert messages to `reports/reports.log` for anyone who falls below a threshold.
